...
function parseJSDocComment(
  ...
  if (paramTags.length > 0) {
    docComment.params = paramTags.map((tag) => {
      const name = tag.comment
        ? String(tag.comment).split(" ")[0]
        : "";
...
function parseFunctionDeclaration(
  ...
  let funcDecl: ts.FunctionDeclaration | ts.ArrowFunction | ts.FunctionExpression;
  let name: string;
  ...
  if (ts.isFunctionDeclaration(node)) {
    ...
    isExported = !!(node.modifiers?.some((m) =>
  ...
  const isAsync = !!(funcDecl.modifiers?.some((m) =>
...
function parseProperty(
  ...
  const isReadonly = !!(node.modifiers?.some((m) =>
...
function parseMethod(
  ...
  const isAsync = !!(node.modifiers?.some((m) =>
    m.kind === ts.SyntaxKind.AsyncKeyword
  ));
  const isStatic = !!(node.modifiers?.some((m) =>
...
function parseClassDeclaration(
  ...
  const extendsClause = node.heritageClauses?.find((c) =>
    c.token === ts.SyntaxKind.ExtendsKeyword
  );
  const extendsType = extendsClause?.types[0]?.getText(sourceFile);

  const implementsClause = node.heritageClauses?.find((c) =>
    c.token === ts.SyntaxKind.ImplementsKeyword
  );
